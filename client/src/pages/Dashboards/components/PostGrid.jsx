...
const PostGrid = ({
	...
	const deleteFile = (i) => {
		let oldFiles = [...files];
		let oldSelectedFiles = [...selectedFiles];
		oldFiles.splice(i, 1);
		oldSelectedFiles.splice(i, 1);
		setFiles(oldFiles);
							...
							>
								{file.type === "image" ? (
									<div className="h-full w-full box-border overflow-hidden items-center flex select-none">
										<img
											src={file.url}
											className="w-full h-full object-contain box-border lg:max-h-full"
										/>
									</div>
								) : (
		...
		return (
			<div className="h-full w-full box-border">
				<div className="w-full box-border h-full relative">
					...
					>
						{files?.map((file, i) => (
							<SwiperSlide key={`${file.url}-${i}`} onClick={stopScroll}>
								{file.type === "image" ? (
									<div
										className="h-full w-full box-border overflow-hidden items-center flex select-none relative"
										onClick={() =>
											modal &&
											(files.length > 1 || files[0].type === "image") &&
											handleShowModal()
