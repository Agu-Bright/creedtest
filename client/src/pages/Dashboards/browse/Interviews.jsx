...
const InterviewCard = ({ name, title, location, id, interview }) => {
	return (
		<Link
			to={"/dashboard/browse/interviews/" + id}
			className="block flex-col border-b p-10 bg-white "
		>
			<div className="flex gap-4">
				...
				<div className="">
					...
					<h3>
						{interview?.jobOccupation}, {interview?.category}
					</h3>
			...
			<Link
				to={"/dashboard/browse/interviews/" + id}
				className="bg-primary-500 py-2 text-center w-full text-white rounded block"
			>
				View
			</Link>
		</Link>
...
const InterviewGroup = ({ title, description, data, loading }) => {
	return (
		<>
...
const Interviews = ({ showFilter, searchValue, filter, location }) => {
		...
		socket &&
			socket.on("new interview", (data) => {
				setInterviews((prev) => {
					...
					const uniquePosts = new Set([data.data.savedInterview, ...prev]);
					return [...uniquePosts];
				});
			});
	});
